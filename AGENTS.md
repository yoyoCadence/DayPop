...
- **Test coverage:** Vitest 目前有 40 個單元案例，涵蓋版本比較、本機資料 envelope／舊 key 保留、公開 Supabase env／HTTP loopback validation、六套 theme token 與 CSS 變數輸出，以及農曆對照與快速新增解析。遠端 DB 的 5 項 rollback pgTAP 流程已驗證 owner RLS、跨帳號隔離、child ownership 與刪除帳號 cascade，security advisor 為 0 警告；登入介面另以 390px Playwright smoke test 驗證 provider 狀態與敏感欄位關閉後清空，正常網路流程 console 0 error／warning。可重複的 repo e2e／CI 尚未建立。
