...
- **Verification commands:** `npm run lint`、`npm run typecheck`、`npm run test`、`npm run build`、`npm run check:build`（需先 build，檢查輸出無遠端依賴且帶 CSP）與 `npm run test:e2e`。這些項目都由 `.github/workflows/ci.yml` 在 PR 與 `main` 的 push 上自動執行，Node major 由 `.nvmrc` 與 `package.json` 的 `engines` 共同固定為 24，CI 不使用任何 secret。Supabase schema 還需可由 `npx supabase db reset` 重建並通過 RLS／migration 驗證；這條 local DB 流程仍依賴 Docker，未納入目前 CI。
  - **跨時區驗證要從 PowerShell 設 `TZ`，並印出 `resolvedOptions().timeZone` 佐證。** Node 本身完全支援 IANA `TZ`（PowerShell 下 `$env:TZ='America/New_York'` 會如實得到 `America/New_York`）。**但從 Bash 工具設定時，含 `/` 的值會在傳給子行程前被整個丟掉** —— `process.env.TZ` 是 `undefined`，於是那次執行**靜默地跑在本機時區**且不報錯：`TZ=America/New_York`、`export TZ=…`、`env TZ=… node` 三種寫法都一樣。不含 `/` 的值（`UTC`、`EST5EDT`）則能通過。因此「`TZ=<五個 IANA 時區>` 各跑一次都過」若是從 Bash 發的，等於同一個時區跑五次；**驗證輸出一定要同時附上該次實際生效的 `Intl.DateTimeFormat().resolvedOptions().timeZone`**，否則無法分辨。另外兩種與 shell 無關、也更有決定性的作法：`vi.setSystemTime()` 釘住瞬間，或在測試內直接指定 `preferences.timezone` 與 `Intl` 的 `timeZone`（見 `CalendarScreen.test.tsx` 的 `today sources` 那組）。Playwright 不受影響，它用 `newPage({ timezoneId })`。CI 跑在 UTC，本機在 UTC+8，**兩者相差一天的那段時間是真的會讓測試紅的**。
