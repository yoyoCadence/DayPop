...
- [ ] **DP-014 — 完成其餘 canonical UI 搬移（第五段起）：** 交接順序第 4 項，同樣不使用 Supabase MCP。日曆管理已由 DP-059 完成；接著依原稿搬移設定的其餘區塊（寵物、一般偏好、通知與預設提醒）與其餘 dialog，並收掉快速新增／事件 sheet 的過渡措施。其中一般偏好與寵物開關需要偏好寫入路徑，依賴 DP-018；AI 區塊依 DP-043、匯入匯出依 DP-056。詳細範圍見 Backlog 的 DP-014。
