...
| --- | --- | --- | --- |
| 月檢視與月份切換 | 可用 | 工程骨架 | 今天、跨月日期與事件／待辦提示正確，且 header、segmented control、快速新增、月格、底部導覽、FAB 與浮動寵物符合原稿。 |
| 週檢視、列表檢視 | 可用 | 原型限定 | 日期範圍、排序與切換後選取狀態一致。 |
| 搜尋與綜覽 | 可用 | 原型限定 | 搜尋事件／待辦，空狀態與結果跳轉可用。 |
| 新增／編輯／刪除事件 | 可用 | 僅新增工程骨架 | 全天與時間事件需有 validation；表單維持原稿 bottom sheet 的資訊層級，編輯／刪除待搬移。 |
| 重複事件與例外 | 可用但模型隱含 | 原型限定 | 依正式 recurrence／exception model 重做並補邊界測試。 |
| 新增／完成／刪除待辦 | 可用 | 新增／完成工程骨架 | 子項、排序、刪除與日期移動待搬移。 |
