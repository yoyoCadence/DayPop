...
| 搜尋與綜覽 | 可用 | 已搬移（DP-058） | 搜尋事件／待辦、閒置與無結果狀態、點結果跳到日曆並開啟；綜覽的類型／年月週切換、筆數與可折疊分組皆可用。Calendar／Sticker 模型已由 DP-012 完成，日曆篩選 chips 與貼圖統計 UI 待 DP-014／055。 |
| 新增／編輯／刪除事件 | 可用 | 新增／編輯／刪除可用（DP-053） | 全天／timed runtime validation 與 canonical contract 已由 DP-012 完成；sheet 外框已是原稿形式，日曆／重複／提醒／地點／時區／邀請／附件欄位待 DP-014／028。 |
| 重複事件與例外 | 可用但模型隱含 | 模型完成、行為原型限定（DP-012） | Canonical Recurrence／EventException 與 DB mapping 已完成；occurrence expansion、單次／全部修改、DST 與 ICS round-trip 待 DP-027。 |
| 日詳情 sheet | 可用 | 已搬移（DP-057） | 點月格開啟；行程列含全天／時間範圍與衝突標籤、空狀態、＋新增事件與待辦清單。貼圖、待辦子項與排序欄位已由 DP-012 完成，操作 UI 待 DP-014／055。 |
...
| 貼圖 | 可用 | 模型完成、UI 原型限定（DP-012） | Canonical Sticker 已同時保留 glyph 與可版本化 asset key；月格／日詳情／綜覽 UI 待 DP-055。 |
