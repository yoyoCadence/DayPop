...
- ~~快速新增直接建立行程~~ **已於 DP-060 收掉**：改為依原稿把解析結果交給事件 sheet 確認，取消即整筆丟棄。仍存不下來的重複與提醒在送出時明講。
- **事件 sheet 只放現有資料模型存得下、而且真的會有行為的欄位**：標題、日曆、全天、日期、開始／結束、地點、備註。重複／時區的 domain、exception、DST 與 ICS 邊界已由 DP-027 完成，但控制項與單次／全部範圍 dialog 仍是 DP-014 的 canonical UI 搬移；提醒（DP-042）、附件（DP-028）與邀請對象（尚無 domain 型別）也只列名說明卡在哪裡，不放假輸入框 — 存得下來但不會有行為，等同假功能。
- **待辦是 sheet 裡的一個模式。** 原檔是從寵物對話泡泡新增待辦（DP-040）。在泡泡搬過來之前把待辦放在這裡，是為了不讓既有能力消失，不是它的正式位置。
- 本專案沒有全域 `button` reset，任何新按鈕都必須自行宣告 `border`，否則會拿到瀏覽器預設的 `2px outset` 黑框（DP-059 的日曆列與 swatch 就踩過這個坑，由 Playwright 檢查抓到）。
- **日曆可見性要在每個「以日期取事件」的入口都套用。** 原稿是靠 `dayEvents()` 這個單一入口達成，DayPop 則分散在各畫面，因此新增任何列出事件的畫面時都要走 `visibleEvents(data)`。DP-059 就漏掉綜覽、由 DP-061 補上；搜尋依原稿刻意不濾。
- 逐日曆顏色已由 DP-059 接上：月格色塊、週檢視色塊、列表與日詳情色條、綜覽色條與搜尋圓點都取自該事件的日曆，文字色統一白色（原檔每個 calendar 的 `text` 都是 `#fff`）。隱藏的日曆由 `visibleEvents()` 在 `CalendarScreen` 一次過濾，對應原檔 `dayEvents()` 的可見性判斷。
...
原檔在拖曳重複事件的單一次發生時會把它拆成獨立事件；DP-027 已完成 occurrence expansion、exception 與 DST-safe 單次修改 helper，但週檢視尚未接上產生後的 occurrence，也尚未搬入 DP-014 的單次／全部範圍 dialog，所以拖曳暫時仍只更新 base event。
