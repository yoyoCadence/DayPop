...
- 每日事件的色塊目前一律用 `--accent`；原檔是每個日曆各自的顏色，要等 DP-012／DP-026 的 Calendar 模型。
- 貼圖格位保留在 cell 版面裡但沒有資料來源，等 DP-012。
...
原檔在拖曳重複事件的單一次發生時會把它拆成獨立事件；DayPop 還沒有 recurrence 模型（DP-012／DP-027），所以拖曳就只更新被拖的那一筆。
...
原稿的日詳情還有三件事需要 DayPop 還沒有的模型，一律保留版面位置並標示負責任務，不放假的控制項：

- 貼圖列與貼圖選擇器 → Sticker 模型 DP-012、UI DP-055
- 待辦子項 → todos 的 `parent_id`（DP-012）
- 待辦拖曳排序 → todos 的 `sort_order`（DP-012）

事件列不顯示地點，因為 domain 還沒有 location 欄位（DP-012）。
...
- **日曆篩選 chips**：原稿一個日曆一個 chip。DayPop 還沒有 Calendar 模型（DP-012），所以只留唯一成立的「全部」，旁邊標示 `依日曆篩選待 DP-012`。
- **綜覽的貼圖分頁**：保留在切換控制項中，但改為顯示明確說明而不是空狀態 — 空狀態會讓人以為「你只是還沒貼」，實際上是沒有資料來源（DP-012／DP-055）。
- 搜尋只比對標題；原稿另外比對地點與備註，兩者都要等 DP-012。
